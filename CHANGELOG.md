...
- `modules:diff` — compare a module against a backup version file by file, using
  a content hash, and report added, modified, removed and unchanged files.
  `--summary` prints counts only.
- `modules:info` — directories, file counts, sizes and backup history for a
...
- `--json` on `modules:prune` (including `--dry-run`) and `modules:doctor`, so
...
- PHPStan/Larastan (level 6) added to the dev dependencies and to CI, run via
  `composer analyse`.
- Test coverage for every console command (`modules:backup`, `modules:delete`,
  `modules:restore`, `modules:prune`, `modules:list`, `modules:path`,
  `modules:doctor`, `modules:backuplist`); previously only `modules:diff` and
  `modules:info` had command-level tests.

### Fixed

- `modules:restore` no longer stages files under `storage/`. The final step of a
...
- A failed move during `modules:restore` now raises an error naming the backup
...
- `modules:list`, `modules:path`, `modules:backup`, `modules:backuplist`,
  `modules:delete`, `modules:restore`, `modules:prune`, `modules:doctor`.
...
- `--dry-run` on `modules:prune` and confirmation prompts on every destructive
...
- `modules:backuplist` takes `--limit` as an option rather than a positional
  argument.
