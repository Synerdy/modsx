...
it('fails when no backups exist', function () {
    $this->artisan('modules:backuplist --json')->assertExitCode(1);
});
...
it('lists the backup versions of a module', function () {
    ...
    $output = json_decode(artisanOutput('modules:backuplist Blog --json'), true);
...
it('limits the result to the newest N versions', function () {
    ...
    $output = json_decode(artisanOutput('modules:backuplist Blog --limit=2 --json'), true);

    expect(array_column($output['Blog'], 'version'))->toBe(['0002', '0003']);
});

it('fails when the named module has no backups', function () {
    $this->artisan('modules:backuplist Ghost --json')->assertExitCode(1);
});
